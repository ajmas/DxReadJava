...
public class DXRead {
    ...
    static class Header {
        ...
        public String toString() {
            ...
            satelliteId + ", " + 
            channelCount + ", " + nightImage + ", " + satelliteType;            
        }
        
        public boolean isPolarSatellite() {
            return satelliteType == -3 || satelliteType == 1 || satelliteType == 2;
        }
    }
    ...
    public Header readHeader(byte[] buffer) {
        ...
        header.satelliteId = Integer.parseInt(line.substring(j, j + 10)
                .trim());
        j += 10;
        header.satelliteType = Integer.parseInt(line.substring(j, j + 10).trim());
        j += 10;
        header.channelCount = Integer.parseInt(line.substring(j, j + 10)
                .trim());
        
        System.out.println("polar: " + header.isPolarSatellite());
    ...
    public void readRecord(Header header, byte[] buffer, BufferedImage image) throws IOException {
        ...
        for (int i = 0; i < npix; i++) {                                    
            ...
            if (image != null) {
                ...
                int col = colorMap.indexToRgb(radiance);
    ...
    public void readFile(File inputFile, File outputFile, boolean mergeImage) throws IOException {
        ...
        try {
            ...
            if (image != null) {                
                for (int i=0; i<255; i++) {
                    for (int j=0; j<10; j++) {
                        image.setRGB(i+30,30+j, colorMap.indexToRgb(i));
                    }
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        // TODO to be replaced with a proper command line parser
        ...
        DXRead dxRead = new DXRead();
        for (File inputFile : inputFiles) {
            dxRead.readFile(inputFile, outputFile, merge);
        }
